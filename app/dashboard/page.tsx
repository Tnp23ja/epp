  ...
  return (
    <main className="min-h-screen p-8">
      ...
      <div className="border-2 border-dashed border-gray-300 rounded-lg p-12 text-center mb-8">
        ...
        <label htmlFor="pdf-upload" className="cursor-pointer bg-blue-500 text-white px-6 py-3 rounded-lg hover:bg-blue-600">
          เลือกไฟล์ PDF
        </label>
        ...
        {message && (
          <p className={`mt-4 ${message.includes("สำเร็จ") ? "text-green-600" : "text-red-500"}`}>
            {message}
          </p>
        )}
      </div>
      ...
      {catalogs.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {catalogs.map((catalog) => (
            <div key={catalog.name} className="border rounded-lg p-4 flex justify-between items-center">
              {/* ชื่อไฟล์ */}
              <p className="font-medium">{catalog.name}</p>
              {/* ปุ่มเปิดไฟล์ */}
              <a
                href={getFileUrl(catalog.name)}
                target="_blank"
